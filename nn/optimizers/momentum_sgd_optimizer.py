...
class MomentumSGDOptimizer(BaseOptimizer):
    ...
    def step(self):
        for i, parameter in enumerate(self.parameters):
            ...
            # print(self.previous_deltas[i])
            # print(parameter.grad.shape)
            # print(parameter.data.shape)
            grad = self.momentum * self.previous_deltas[i] + parameter.grad + self.weight_decay * parameter.data
            parameter.data = parameter.data - self.learning_rate * grad
            self.previous_deltas[i] = grad
